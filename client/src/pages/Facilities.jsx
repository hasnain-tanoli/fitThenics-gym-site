      ...
      <div className="bg-background text-foreground">
        ...
        <header className="py-24 text-center bg-secondary">
          <AnimatedSection animation="fade-in-up">
            <h1 className="font-heading text-4xl md:text-5xl font-bold text-foreground">
              Explore Our <span className="text-primary">World-Class</span> Facilities
            </h1>
        ...
        <section className="py-20 px-4 bg-background">
          <div className="max-w-7xl mx-auto">
            {/* Main Display */}
            <AnimatedSection animation="fade-in-up" className="mb-8 relative h-[60vh] w-full overflow-hidden rounded-lg shadow-lg shadow-primary/20">
              ...
              <div className="absolute bottom-0 left-0 p-8 bg-black/70 rounded-tr-lg"> {/* Semi-transparent black text box */}
                <h2 className="font-heading text-4xl text-primary">{facilities[selectedImageIndex].title}</h2>
                <p className="font-body text-lg text-secondary-foreground max-w-lg">{facilities[selectedImageIndex].description}</p>
              </div>
            </AnimatedSection>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {facilities.map((facility, index) => (
                  <motion.div
                    ...
                    className={`relative overflow-hidden rounded-lg cursor-pointer border-2 transition-all duration-300 hover:scale-105 ${selectedImageIndex === index ? 'border-primary' : 'border-transparent'}`}
                    whileHover={{ scale: 1.05 }}
                  >
                    <img src={facility.imgSrc} alt={facility.title} className="w-full h-32 object-cover" />
                    <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                      <h3 className="font-heading text-xl text-foreground">{facility.title}</h3>
                    </div>
                  </motion.div>
                ))}
              </div>
        ...
        <section className="py-20 px-4 bg-secondary">
          <div className="max-w-6xl mx-auto">
            <AnimatedSection animation="fade-in-up" className="text-center mb-16">
              <h2 className="font-heading text-4xl md:text-5xl font-bold text-foreground">Key <span className="text-primary">Amenities</span></h2>
            </AnimatedSection>
