        ...
        >
          {testimonials.map((testimonial, index) => (
            <SwiperSlide key={index}>
              <motion.div
                whileHover={{ y: -8, boxShadow: '0 20px 25px -5px rgba(255, 234, 0, 0.1), 0 10px 10px -5px rgba(255, 234, 0, 0.04)' }}
                className="bg-secondary rounded-lg p-8 h-full flex flex-col border border-light/10"
              >
                <Quote className="w-10 h-10 text-accent/50 mb-4" />
                <p className="font-body text-light/80 mb-6 flex-grow">
                  "{testimonial.text}"
                </p>
                <div className="flex items-center mt-auto">
                  <img src={testimonial.image} alt={testimonial.name} className="w-14 h-14 rounded-full object-cover mr-4 border-2 border-accent" />
                  <div>
